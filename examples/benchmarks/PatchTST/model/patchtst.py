...
class PatchTST(Model):
    ...
    def train_epoch(self, train_loader):
        ...
        for _, (input_x, weight_x) in enumerate(tqdm(train_loader, mininterval=2)):
            ...
            if self.device.type == 'cuda':
                ...
                emb_mark = emb_mark.float().to(self.device)
        ...
        # Log memory usage at the end of epoch
        if self.device.type == 'cuda':
            current_memory = torch.cuda.memory_allocated(self.device) / 1024 / 1024  # Convert to MB
            self.logger.info(f"GPU memory usage at end of epoch: {current_memory:.2f} MB")
    ...
    def test_epoch(self, data_loader):
        ...
        try:
            ...
            if self.device.type == 'cuda' and inference_times:
        ...
        for _, input_x in enumerate(tqdm(test_loader, mininterval=2)):
            ...
            with torch.no_grad():
                try:
                    ...
                    # 在发生错误时，添加一个与输入形状相同的零张量
                    batch_size = seq_x.size(0)
                    dummy_output = torch.zeros((batch_size, 1, 1), device=self.device)
                    preds.append(dummy_output.detach().cpu().numpy())

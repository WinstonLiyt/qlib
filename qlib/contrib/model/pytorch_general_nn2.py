...
class ICLoss(nn.Module):
    ...
    Minimize negative Pearson correlation between prediction and label.
    """
    def __init__(self):
    ...
    def forward(self, pred, label, weight=None):
        ...
        return -ic * 100.0  # minimize negative IC
...
class GeneralPTNN(Model):
    ...
    def loss_fn(self, pred, label, weight=None):
        ...
        if self.loss == "mse":
            ...
            if not hasattr(self, "_ic_loss"):
                self._ic_loss = ICLoss()
            return self._ic_loss(pred, label)
        else:
    ...
    def train_epoch(self, data_loader, epoch=0):
        ...
        for data, weight in pbar:
            feature, label = self._get_fl(data)
            pred = self.dnn_model(feature.float())

            # 动态loss切换
            if epoch < self.warmup_epoch:
                loss = self.mse(pred, label, weight.to(self.device))  # warmup阶段用MSE
            else:
                loss = self.loss_fn(pred, label, weight.to(self.device))  # 正常阶段用loss_fn
    ...
    def test_epoch(self, data_loader):
        ...
        for data, weight in pbar:
            ...
            with torch.no_grad():
                ...
                # Update progress bar with current loss and score
                pbar.set_postfix({
                    "loss": f"{loss.item():.6f}",
    ...
    def fit(
        ...
        for step in range(self.n_epochs):
            ...
            self.logger.info("training...")
            self.train_epoch(train_loader, epoch=step)
